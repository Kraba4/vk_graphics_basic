...
void SimpleShadowmapRender::UpdateUniformBuffer(float a_time)
{
  ...
  m_uniforms.oldProjectionViewMatrix = m_uniforms.projectionViewMatrix;
  m_uniforms.projectionViewMatrix = m_worldViewProj;
  m_uniforms.eyeAndJitter = shader_vec4(m_cam.pos.x, m_cam.pos.y, m_cam.pos.z, float(int(m_uniforms.eyeAndJitter.w + 1) % 16));
