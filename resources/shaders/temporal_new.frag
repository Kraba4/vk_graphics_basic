...
layout(location = 1) out vec4 out_historyColor;
...
void main()
{  
  vec4 posLightClipSpace = Params.lightMatrix*vec4(surf.wPos, 1.0f);
  ...
  out_sceneColor = vec4(newColor, 1);
  out_historyColor = outOfOldView ? vec4(newColor, 1) : oldColor;
}
